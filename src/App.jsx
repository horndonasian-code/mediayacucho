...
const SPECIALTY_CONFIG = {
  ...
  "Otorrinolaringología":  { color: "#8b5cf6", bg: "rgba(139,92,246,0.12)",  icon: "👂" },
...
export default function App() {
  ...
  const [misCitasPhone, setMisCitasPhone] = useState("");
  const [misCitasData, setMisCitasData] = useState(null);
  const [misCitasLoading, setMisCitasLoading] = useState(false);
    ...
    <div style={{ ...T.app, background: (view==="home"||view==="doctors"||view==="chat"||view==="booking") ? "#ffffff" : T.app.background }}>
      ...
      <header style={{ ...T.header, background: (view==="home"||view==="doctors"||view==="chat"||view==="booking") ? "rgba(255,255,255,0.95)" : T.header.background, borderBottom: (view==="home"||view==="doctors"||view==="chat"||view==="booking") ? "1px solid rgba(14,165,233,0.2)" : T.header.borderBottom }}>
        ...
        <nav className="desktop-nav" style={{ display: "flex", gap: 8, alignItems:"center" }}>
          ...
          <button style={T.navBtn(view==="mis-citas", view==="home"||view==="doctors"||view==="chat"||view==="booking"||view==="mis-citas")} onClick={() => setView("mis-citas")}>
            <span style={{ display:"flex", alignItems:"center", gap:5 }}>
      ...
      {view === "mis-citas" && (
        <div style={{ maxWidth:600, margin:"0 auto", padding:"40px 24px" }}>
          <h2 style={{ fontSize:28, fontWeight:800, color:"#082f49", margin:"0 0 8px" }}>Mis citas</h2>
          <p style={{ color:"#475569", fontSize:14, margin:"0 0 28px" }}>Ingresa tu número de WhatsApp para ver y gestionar tus citas.</p>

          <div style={{ display:"flex", gap:10, marginBottom:24 }}>
            <input
              style={{ flex:1, padding:"12px 16px", border:"1.5px solid #e2e8f0", borderRadius:12, fontFamily:"inherit", fontSize:15, outline:"none" }}
            ...
            <button
              ...
              onClick={async () => {
                ...
                setMisCitasLoading(true);
                try {
                  const phone = misCitasPhone.replace(/\s/g, "");
                  const res = await fetch(`${SUPABASE_URL}/rest/v1/appointments?patient_phone=eq.${encodeURIComponent(phone)}&order=date.desc&select=*`, {
                    headers: { "apikey": SUPABASE_KEY, "Authorization": `Bearer ${SUPABASE_KEY}` }
                  });
                  const data = await res.json();
                  setMisCitasData(data || []);
                } catch(e) { setMisCitasData([]); }
                setMisCitasLoading(false);
              }}>
              Buscar
            </button>
          </div>

          {misCitasLoading && <p style={{ color:"#0369a1", textAlign:"center" }}>Buscando tus citas...</p>}

          {misCitasData !== null && !misCitasLoading && (
            misCitasData.length === 0
              ? <div style={{ textAlign:"center", padding:40, color:"#64748b" }}>
                  <div style={{ fontSize:40, marginBottom:12 }}>📭</div>
                  <p>No encontramos citas con este número.</p>
                </div>
              : <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                  {misCitasData.map(appt => {
                    const doc = doctors.find(d => d.id === appt.doctor_id);
                    const apptDateTime = new Date(`${appt.date}T${appt.time}`);
                    const hoursLeft = (apptDateTime - new Date()) / (1000 * 60 * 60);
                    const canCancel = appt.status !== "cancelada" && appt.status !== "completada" && hoursLeft > 0;
                    const statusColors = { pendiente:"#f59e0b", confirmada:"#10b981", cancelada:"#dc2626", completada:"#6b7280" };
                    const statusLabels = { pendiente:"Pendiente", confirmada:"Confirmada", cancelada:"Cancelada", completada:"Completada" };
                    return (
                      <div key={appt.id} style={{ background:"#fff", border:"1px solid #e0f2fe", borderRadius:16, padding:20, boxShadow:"0 2px 12px rgba(15,23,42,0.05)" }}>
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
                          <div>
                            <div style={{ fontWeight:700, fontSize:16, color:"#082f49" }}>{doc?.name || "Médico"}</div>
                            <div style={{ fontSize:13, color:"#64748b" }}>{doc?.specialty}</div>
                          </div>
                          <span style={{ padding:"4px 12px", borderRadius:20, background:`${statusColors[appt.status]}18`, color:statusColors[appt.status], fontSize:12, fontWeight:700 }}>
                            {statusLabels[appt.status] || appt.status}
                          </span>
                        </div>
                        <div style={{ fontSize:13, color:"#475569", marginBottom:12 }}>
                          <div>📅 {appt.date} · 🕐 {appt.time}</div>
                          <div>📱 {appt.modalidad === "virtual" ? "Consulta virtual" : "Presencial"}</div>
                          {doc?.address && <div>📍 {doc.address}</div>}
                        </div>
                        {canCancel && (
                          <button
                            style={{ padding:"8px 18px", background:"#fef2f2", border:"1px solid #fecaca", color:"#dc2626", borderRadius:10, cursor:"pointer", fontFamily:"inherit", fontSize:13, fontWeight:700 }}
                            onClick={async () => {
                              if (!window.confirm(`¿Cancelar tu cita del ${appt.date} a las ${appt.time}?\n${hoursLeft >= 24 ? "✅ Serás reembolsado S/. 20" : "⚠️ No habrá reembolso (menos de 24h)"}`)) return;
                              ...
                              // Notificar admin
                              const msgAdmin = `❌ *CANCELACIÓN DE PACIENTE - MediAyacucho*\n\n👤 ${appt.patient_name}\n📞 ${appt.patient_phone}\n📅 ${appt.date} · ${appt.time}\n👨‍⚕️ ${doc?.name}\n💰 ${hoursLeft >= 24 ? `REEMBOLSAR S/. ${AVANCE.monto} al ${appt.patient_phone}` : "SIN REEMBOLSO (menos de 24h)"}`;
                              setTimeout(() => window.open(`https://wa.me/51913330712?text=${encodeURIComponent(msgAdmin)}`, "_blank"), 300);
                              setMisCitasData(prev => prev.map(a => a.id === appt.id ? {...a, status:"cancelada"} : a));
                            }}>
                            ❌ Cancelar esta cita
                          </button>
                        )}
            ...
            <div style={{ maxWidth:540, margin:"0 auto" }}>
              ...
              <div style={{ background:"#ffffff", border:"1px solid #bae6fd", borderRadius:16, padding:24, marginBottom:24, boxShadow:"0 8px 28px rgba(15,23,42,0.06)" }}>
                ...
                <div style={{ marginTop:16, display:"flex", gap:10 }}>
                  {selectedDoctor.specialty === "Enfermero a domicilio" ? (
                    <div style={{ flex:1, padding:"10px 0", borderRadius:10, border:"2px solid #10b981", background:"#ecfdf5", color:"#065f46", fontSize:14, fontWeight:700, textAlign:"center" }}>
          ...
          <div style={{ maxWidth:1100, margin:"0 auto" }}>
            <div className="footer-grid" style={{ display:"grid", gridTemplateColumns:"2fr 1fr 1fr 1fr", gap:40, marginBottom:48 }}>
              ...
              <div>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                  {[
                    { svg:<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347zM12 0C5.373 0 0 5.373 0 12c0 2.116.554 4.103 1.523 5.824L0 24l6.342-1.498A11.955 11.955 0 0 0 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22c-1.894 0-3.669-.497-5.211-1.371L3 21.5l.894-3.654A10 10 0 0 1 2 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z"/></svg>, color:"#25D366", label:"WhatsApp", val:"913 330 712", url:"https://wa.me/51913330712" },
                    { svg:<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>, color:"#7dd3fc", label:"Email", val:"info@mediayacucho.pe", url:"mailto:info@mediayacucho.pe" },
                    { svg:<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>, color:"#f87171", label:"Ciudad", val:"Ayacucho, Perú", url:null },
                    { svg:<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>, color:"#bae6fd", label:"Horario", val:"Lun–Sáb 8am–8pm", url:null },
                  ].map((item,i)=>(
                    <div key={i} style={{ display:"flex", gap:8, alignItems:"flex-start" }}>
                      <span style={{ marginTop:1, color:item.color }}>{item.svg}</span>
                      <div>
