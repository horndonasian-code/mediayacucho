...
function PaymentModal({ doctor, bookingData, onSuccess, onClose, isMembership }) {
  ...
  const s = {
    ...
    box: { background: "#0a1628", border: "1px solid rgba(82,183,136,0.3)", borderRadius: 24, padding: 32, maxWidth: 480, width: "100%", maxHeight: "92vh", overflowY: "auto" },
    ...
    inp: (err) => ({ width: "100%", background: "rgba(255,255,255,0.06)", border: `1px solid ${err ? "#ff6b6b" : "rgba(82,183,136,0.25)"}`, borderRadius: 10, padding: "11px 14px", color: "#e8f4e8", fontSize: 14, fontFamily: "inherit", outline: "none", boxSizing: "border-box" }),
    label: { display: "block", fontSize: 11, color: "#74C69D", marginBottom: 5, letterSpacing: 0.8, textTransform: "uppercase" },
    errTxt: { color: "#ff6b6b", fontSize: 11, marginTop: 3, display: "block" },
    payBtn: { width: "100%", padding: "14px 0", background: "linear-gradient(135deg, #2D6A4F, #52B788)", color: "#fff", border: "none", borderRadius: 12, cursor: "pointer", fontSize: 16, fontWeight: 700, fontFamily: "inherit", marginTop: 16, boxShadow: "0 6px 24px rgba(82,183,136,0.3)" },
    backBtn: { background: "none", border: "none", color: "#52B788", cursor: "pointer", fontSize: 13, padding: "0 0 16px", fontFamily: "inherit" },
    bankRow: { display: "flex", justifyContent: "space-between", padding: "10px 0", borderBottom: "1px solid rgba(82,183,136,0.1)", fontSize: 14 },
    copyBtn: { background: "rgba(82,183,136,0.15)", border: "1px solid rgba(82,183,136,0.3)", color: "#52B788", borderRadius: 6, padding: "3px 10px", fontSize: 11, cursor: "pointer", fontFamily: "inherit" },
  ...
  if (step === "processing") return (
    <div style={s.overlay}><div style={{ ...s.box, textAlign: "center", padding: "60px 32px" }}>
      ...
      <h3 style={{ color: "#52B788", fontSize: 22, margin: "0 0 8px" }}>Procesando pago...</h3>
      <p style={{ color: "#74C69D" }}>Por favor no cierres esta ventana</p>
      <div style={{ marginTop: 24, height: 4, background: "rgba(82,183,136,0.15)", borderRadius: 2, overflow: "hidden" }}>
        <div style={{ height: "100%", background: "linear-gradient(90deg,#2D6A4F,#52B788)", borderRadius: 2, animation: "prog 2s ease forwards" }} />
      </div>
    </div></div>
  );
  ...
  if (step === "done") return (
    <div style={s.overlay}><div style={{ ...s.box, textAlign: "center", padding: "60px 32px" }}>
      ...
      <h3 style={{ color: "#52B788", fontSize: 24, margin: "0 0 8px" }}>{isMembership ? "¡Membresía registrada!" : "¡Pago exitoso!"}</h3>
      <p style={{ color: "#74C69D" }}>S/. {amount} {isMembership ? "— Verificaremos tu pago en breve" : "pagados correctamente"}</p>
    </div></div>
  );
    ...
    <div style={s.overlay}>
      <div style={s.box}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <div>
            <h3 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: "#e8f4e8" }}>💳 {isMembership ? "Pagar membresía" : "Pagar cita médica"}</h3>
            <p style={{ margin: "4px 0 0", fontSize: 12, color: "#74C69D" }}>Transacción segura — MediAyacucho</p>
          </div>
          <button onClick={onClose} style={{ background: "none", border: "none", color: "#74C69D", fontSize: 22, cursor: "pointer" }}>✕</button>
        </div>

        <div style={{ background: "rgba(82,183,136,0.07)", border: "1px solid rgba(82,183,136,0.15)", borderRadius: 12, padding: 16, marginBottom: 20 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              <div style={{ fontSize: 14, color: "#e8f4e8", fontWeight: 700 }}>{isMembership ? "Plan Profesional — MediAyacucho" : doctor?.name}</div>
              <div style={{ fontSize: 12, color: "#74C69D" }}>{isMembership ? "Membresía mensual" : `${bookingData?.date} · ${bookingData?.time}`}</div>
            </div>
            <div style={{ fontSize: 26, fontWeight: 700, color: "#52B788" }}>S/. {amount}</div>
          </div>
        </div>

        {step === "choose" && (
          <>
            <p style={{ fontSize: 13, color: "#74C69D", margin: "0 0 14px" }}>Selecciona tu método de pago:</p>
            {METHODS.map(m => (
              <button key={m.id} style={s.methodBtn(method === m.id, m.color)} onClick={() => setMethod(m.id)}>
                <span style={{ fontSize: 26 }}>{m.icon}</span>
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 700, color: method === m.id ? "#e8f4e8" : "#95D5B2", fontSize: 15 }}>{m.label}</div>
                  <div style={{ fontSize: 12, color: "#74C69D" }}>{m.sub}</div>
                </div>
        ...
        {step === "form" && method === "yape" && (
          <>
            ...
            <div style={s.steps}>
              {[["Abre tu app Yape en tu celular","🟣"],["Yapea S/. "+amount+" al número 913 330 712","🟣"],["Concepto: "+concept,"🟣"],["Envía la captura por WhatsApp","🟣"]].map(([txt,c],i)=>(
                <div key={i} style={s.stepRow(c)}>
                  ...
                  <span style={{ color: "#95D5B2", fontSize: 14 }}>{txt}</span>
                </div>
              ))}
            </div>
            <div style={s.numBox("#6C3FC5")}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontSize: 28, fontWeight: 700, color: "#e8f4e8", letterSpacing: 3 }}>913 330 712</span>
              ...
              <div style={{ marginTop: 10, display: "flex", justifyContent: "space-between", fontSize: 13 }}>
                <span style={{ color: "#74C69D" }}>Monto:</span><strong style={{ color: "#52B788" }}>S/. {amount}</strong>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, marginTop: 6 }}>
                <span style={{ color: "#74C69D" }}>Concepto:</span><strong style={{ color: "#e8f4e8" }}>{concept}</strong>
              </div>
            </div>
            <button style={s.waBtn} onClick={() => { const msg = `🟣 *COMPROBANTE YAPE - MediAyacucho*\n\n💰 Monto: S/. ${amount}\n📋 Concepto: ${concept}\n\n📎 Adjunto captura de Yape.`; window.open(`https://wa.me/51913330712?text=${encodeURIComponent(msg)}`, "_blank"); }}>
        ...
        {step === "form" && method === "culqi" && (
          <>
            ...
            <div style={{ background: "rgba(82,183,136,0.06)", borderRadius: 10, padding: "10px 14px", fontSize: 12, color: "#74C69D", marginBottom: 4 }}>
              🔒 Pago procesado por <strong style={{ color: "#52B788" }}>Culqi</strong> — certificación PCI DSS.
            </div>
        ...
        {step === "form" && method === "transferencia" && (
          <>
            ...
            {[{ bank:"BCP", num:"194-2345678-0-12", cci:"002-194-00234567801234-56" },{ bank:"BBVA", num:"0011-0198-0123456789", cci:"011-198-000123456789-00" }].map(b=>(
              <div key={b.bank} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(82,183,136,0.12)", borderRadius: 12, padding: 16, marginBottom: 12 }}>
                <div style={{ fontWeight: 700, color: "#52B788", fontSize: 15, marginBottom: 10 }}>🏦 {b.bank}</div>
                <div style={s.bankRow}><span style={{ color: "#74C69D", fontSize: 13 }}>Cuenta</span><span style={{ color: "#e8f4e8", fontSize: 13 }}>{b.num}</span></div>
                <div style={{ ...s.bankRow, borderBottom:"none", alignItems:"flex-start" }}>
                  <span style={{ color: "#74C69D", fontSize: 13 }}>CCI</span>
                  <div style={{ textAlign:"right" }}>
                    <span style={{ color: "#e8f4e8", fontSize: 12 }}>{b.cci}</span><br />
...
function NotificationPanel({ bookingData, doctor, onClose }) {
  ...
  const s = {
    ...
    box: { background: "#0d2137", border: "1px solid rgba(82,183,136,0.35)", borderRadius: 24, padding: 32, maxWidth: 500, width: "100%", maxHeight: "90vh", overflowY: "auto" },
    tab: (a) => ({ flex: 1, padding: "10px 0", borderRadius: 10, border: `1px solid ${a ? "#52B788" : "rgba(82,183,136,0.25)"}`, background: a ? "rgba(82,183,136,0.15)" : "transparent", color: a ? "#52B788" : "#74C69D", cursor: "pointer", fontFamily: "inherit", fontSize: 14 }),
    nCard: (d) => ({ background: d ? "rgba(82,183,136,0.08)" : "rgba(255,255,255,0.04)", border: `1px solid ${d ? "rgba(82,183,136,0.4)" : "rgba(82,183,136,0.15)"}`, borderRadius: 14, padding: 16, marginBottom: 12 }),
    preview: { background: "rgba(0,0,0,0.25)", borderRadius: 8, padding: 10, fontSize: 11, color: "#95D5B2", whiteSpace: "pre-wrap", lineHeight: 1.5, maxHeight: 90, overflowY: "auto", marginBottom: 10 },
    ...
    btnSms: { width: "100%", padding: "10px 0", background: "linear-gradient(135deg, #2D6A4F, #52B788)", color: "#fff", border: "none", borderRadius: 10, cursor: "pointer", fontSize: 13, fontWeight: 700, fontFamily: "inherit" },
  };
  return (
    <div style={s.overlay}>
      <div style={s.box}>
        <h3 style={{ fontSize: 20, fontWeight: 700, color: "#52B788", margin: "0 0 4px" }}>📲 Notificaciones</h3>
        <p style={{ color: "#74C69D", fontSize: 13, margin: "0 0 20px" }}>Envía confirmaciones a {bookingData.patient_name} y al médico</p>
        ...
        {notifs.map(n => (
          <div key={n.key} style={s.nCard(sent[n.key])}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
              <span style={{ fontSize: 14, fontWeight: 700, color: "#e8f4e8" }}>{n.icon} {n.title}</span>
              {sent[n.key] && <span style={{ background: "rgba(82,183,136,0.2)", color: "#52B788", borderRadius: 6, padding: "2px 8px", fontSize: 11 }}>Enviado ✓</span>}
            </div>
        ...
        <button style={{ width: "100%", marginTop: 16, padding: "12px 0", background: "transparent", border: "1.5px solid rgba(82,183,136,0.4)", color: "#52B788", borderRadius: 12, cursor: "pointer", fontSize: 15, fontFamily: "inherit" }} onClick={onClose}>Cerrar</button>
      </div>
    </div>
  );
}
...
function DoctorDashboard({ doctor, onExit }) {
  ...
  const statusColor = { confirmada: "#52B788", pendiente: "#F4A261", completada: "#74C69D", cancelada: "#ff6b6b" };
  const statusBg = { confirmada: "rgba(82,183,136,0.15)", pendiente: "rgba(244,162,97,0.15)", completada: "rgba(116,198,157,0.1)", cancelada: "rgba(255,107,107,0.1)" };
  ...
  const s = {
    wrap: { minHeight: "100vh", background: "linear-gradient(135deg, #0a1628 0%, #0d2137 100%)", fontFamily: "'Crimson Pro', Georgia, serif", color: "#e8f4e8" },
    topbar: { background: "rgba(10,22,40,0.95)", borderBottom: "1px solid rgba(82,183,136,0.2)", padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64, position: "sticky", top: 0, zIndex: 50 },
    avatar: { width: 40, height: 40, borderRadius: 10, background: doctor.color || "#2D6A4F", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 14, color: "#fff" },
    sidebar: { width: 220, minHeight: "calc(100vh - 64px)", background: "rgba(0,0,0,0.2)", borderRight: "1px solid rgba(82,183,136,0.1)", padding: "24px 0", position: "fixed", top: 64 },
    sideBtn: (a) => ({ display: "flex", alignItems: "center", gap: 10, padding: "12px 24px", cursor: "pointer", background: a ? "rgba(82,183,136,0.12)" : "transparent", borderLeft: a ? "3px solid #52B788" : "3px solid transparent", color: a ? "#52B788" : "#74C69D", fontSize: 14, border: "none", width: "100%", textAlign: "left", fontFamily: "inherit" }),
    ...
    kpiLabel: { fontSize: 12, color: "#74C69D", letterSpacing: 0.5, textTransform: "uppercase" },
    card: { background: "rgba(255,255,255,0.04)", border: "1px solid rgba(82,183,136,0.12)", borderRadius: 16, padding: 24, marginBottom: 20 },
    cardTitle: { fontSize: 16, fontWeight: 700, color: "#e8f4e8", margin: "0 0 16px" },
    ...
    selectBtn: { background: "rgba(255,255,255,0.06)", border: "1px solid rgba(82,183,136,0.2)", borderRadius: 8, padding: "4px 10px", color: "#74C69D", fontSize: 12, cursor: "pointer", fontFamily: "inherit" },
    filterPill: (a) => ({ padding: "6px 14px", borderRadius: 20, border: `1px solid ${a ? "#52B788" : "rgba(82,183,136,0.25)"}`, background: a ? "rgba(82,183,136,0.15)" : "transparent", color: a ? "#52B788" : "#74C69D", cursor: "pointer", fontSize: 12, fontFamily: "inherit" }),
    toggleBtn: (on) => ({ padding: "8px 20px", borderRadius: 10, border: "none", background: on ? "linear-gradient(135deg, #2D6A4F, #52B788)" : "rgba(255,100,100,0.15)", color: on ? "#fff" : "#ff6b6b", cursor: "pointer", fontFamily: "inherit", fontSize: 13, fontWeight: 700 }),
    inp: { width: "100%", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(82,183,136,0.25)", borderRadius: 8, padding: "8px 12px", color: "#e8f4e8", fontSize: 14, fontFamily: "inherit", outline: "none", boxSizing: "border-box", marginBottom: 10 },
    saveBtn: { padding: "10px 24px", background: "linear-gradient(135deg, #2D6A4F, #52B788)", color: "#fff", border: "none", borderRadius: 10, cursor: "pointer", fontFamily: "inherit", fontSize: 14, fontWeight: 700 },
    lbl: { display: "block", fontSize: 12, color: "#74C69D", marginBottom: 4 },
  };
    ...
    <div style={s.wrap}>
      ...
      <div style={s.topbar}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            <div style={{ fontWeight: 700, color: "#e8f4e8", fontSize: 15 }}>{doctor.name}</div>
            <div style={{ fontSize: 11, color: "#52B788" }}>{doctor.specialty} · Panel Médico</div>
          </div>
        </div>
        <div style={{ display: "flex", gap: 12 }}>
          <button style={s.toggleBtn(isAvailable)} onClick={toggleAvailable}>{isAvailable ? "🟢 Disponible" : "🔴 No disponible"}</button>
          <button onClick={onExit} style={{ padding: "8px 16px", background: "transparent", border: "1px solid rgba(82,183,136,0.3)", color: "#74C69D", borderRadius: 8, cursor: "pointer", fontFamily: "inherit", fontSize: 13 }}>← Salir</button>
        </div>
      </div>

      <div style={s.sidebar}>
        {navItems.map(n => <button key={n.id} style={s.sideBtn(tab === n.id)} onClick={() => setTab(n.id)}><span>{n.icon}</span> {n.label}</button>)}
        <div style={{ padding: "24px 24px 0", borderTop: "1px solid rgba(82,183,136,0.1)", marginTop: 24 }}>
          <div style={{ fontSize: 11, color: "#74C69D", marginBottom: 4 }}>MEMBRESÍA ACTIVA</div>
          <div style={{ fontSize: 13, color: "#52B788", fontWeight: 700 }}>Plan Profesional</div>
          <div style={{ fontSize: 12, color: "#74C69D" }}>S/. 99/mes</div>
        </div>
      </div>
      ...
      <div style={s.main}>
        ...
        {tab === "overview" && (
          <>
            ...
            <div style={s.kpiGrid}>
              <div style={s.kpiCard("#52B788")}><span style={s.kpiLabel}>Citas hoy</span><span style={s.kpiNum("#52B788")}>{todayAppts.length}</span></div>
              ...
              <div style={s.kpiCard("#74C69D")}><span style={s.kpiLabel}>Completadas</span><span style={s.kpiNum("#74C69D")}>{completedCount}</span></div>
              <div style={s.kpiCard("#52B788")}><span style={s.kpiLabel}>Ingresos</span><span style={s.kpiNum("#52B788")}>S/. {totalIncome}</span></div>
            ...
            <div style={s.card}>
              ...
              {loadingAppts ? <p style={{ color: "#74C69D" }}>Cargando...</p> : todayAppts.length === 0
                ? <p style={{ color: "#74C69D", fontSize: 14 }}>No hay citas para hoy.</p>
                : todayAppts.map(a => (
                  <div key={a.id} style={s.apptRow(a.status)}>
                    <div>
                      <span style={{ fontWeight: 700, color: "#e8f4e8" }}>{a.time} · {a.patient_name}</span>
                      <span style={{ display: "block", fontSize: 12, color: "#74C69D" }}>📞 {a.patient_phone}</span>
                    </div>
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                      <span style={s.statusPill(a.status)}>{a.status}</span>
                      {a.status === "pendiente" && <button style={{ ...s.selectBtn, color: "#52B788", borderColor: "#52B788" }} onClick={() => updateStatus(a.id, "confirmada")}>Confirmar</button>}
                      {a.status === "confirmada" && <button style={{ ...s.selectBtn, color: "#74C69D" }} onClick={() => updateStatus(a.id, "completada")}>Completar</button>}
                    </div>
                  </div>
                ))
              }
        ...
        {tab === "appointments" && (
          <>
            ...
            <div style={s.card}>
              {loadingAppts ? <p style={{ color: "#74C69D" }}>Cargando desde Supabase...</p> : filtered.length === 0
                ? <p style={{ color: "#74C69D", fontSize: 14 }}>No hay citas con este filtro.</p>
                : filtered.map(a => (
                  <div key={a.id} style={s.apptRow(a.status)}>
                    <div style={{ flex: 1 }}>
                      <span style={{ fontWeight: 700, color: "#e8f4e8" }}>{a.patient_name}</span>
                      <span style={{ display: "block", fontSize: 12, color: "#74C69D" }}>📅 {a.date} · 🕐 {a.time} · 📞 {a.patient_phone}</span>
                    </div>
                    <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                      <span style={s.statusPill(a.status)}>{a.status}</span>
                      {a.status === "pendiente" && <>
                        <button style={{ ...s.selectBtn, color:"#52B788", borderColor:"#52B788" }} onClick={() => updateStatus(a.id,"confirmada")}>✓ Confirmar</button>
                        <button style={{ ...s.selectBtn, color:"#ff6b6b", borderColor:"#ff6b6b" }} onClick={() => updateStatus(a.id,"cancelada")}>✗ Cancelar</button>
                      </>}
                      {a.status === "confirmada" && <>
                        <button style={{ ...s.selectBtn, color:"#74C69D" }} onClick={() => updateStatus(a.id,"completada")}>✓ Completar</button>
                        <button style={{ ...s.selectBtn, color:"#ff6b6b", borderColor:"#ff6b6b" }} onClick={() => updateStatus(a.id,"cancelada")}>✗ Cancelar</button>
                      </>}
        ...
        {tab === "analytics" && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
              {[["📊 Citas por mes", MONTHLY_DATA, "citas", "#52B788", maxCitas], ["💰 Ingresos (S/.)", MONTHLY_DATA, "ingresos", "#F4A261", 2100]].map(([title, data, key, color, max]) => (
                  ...
                  <div style={{ display: "flex", alignItems: "flex-end", gap: 10, height: 140, paddingTop: 16 }}>
                    {data.map(d => (
                      <div key={d.mes} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                        ...
                        <span style={{ fontSize: 10, color: "#74C69D" }}>{d.mes}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16 }}>
              {[
                { label: "Total citas", value: appointments.length, color: "#52B788" },
          ...
          <>
            ...
            <div style={s.card}>
              <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 24 }}>
                <div style={{ width: 72, height: 72, borderRadius: 18, background: doctor.color || "#2D6A4F", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 24, color: "#fff" }}>{doctor.img || initials(doctor.name)}</div>
                <div>
                  <h3 style={{ margin: 0, fontSize: 20 }}>{profileData.name}</h3>
                  <p style={{ margin: "4px 0 0", color: "#74C69D" }}>{profileData.specialty} · {profileData.price}/consulta</p>
                  {profileData.address && <p style={{ margin: "4px 0 0", fontSize: 13, color: "#52B788" }}>📍 {profileData.address}</p>}
                  <div style={{ marginTop: 6 }}>⭐ {doctor.rating} · {isAvailable ? <span style={{ color: "#52B788" }}>🟢 Disponible</span> : <span style={{ color: "#ff6b6b" }}>🔴 No disponible</span>}</div>
                </div>
              </div>
              {editProfile ? (
                <>
                  ...
                  <p style={{ margin:"-6px 0 12px", fontSize: 11, color: "#74C69D" }}>📍 Visible para los pacientes al reservar</p>
                  <div style={{ display: "flex", gap: 10 }}>
                    ...
                    <button style={{ ...s.saveBtn, background:"transparent", border:"1px solid rgba(82,183,136,0.3)", color:"#74C69D" }} onClick={() => setEditProfile(false)}>Cancelar</button>
                  </div>
                </>
              ) : (
            ...
            <div style={s.card}>
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                {(doctor.schedule || []).map(h => <span key={h} style={{ padding: "8px 16px", borderRadius: 20, background: "rgba(82,183,136,0.15)", border: "1px solid rgba(82,183,136,0.3)", color: "#52B788", fontSize: 14 }}>{h}</span>)}
              </div>
            </div>
          </>
        )}
        ...
        {tab === "ai" && (
          <>
            ...
            <p style={{ color: "#74C69D", margin: "0 0 24px" }}>La IA analiza tus datos reales y te da recomendaciones personalizadas</p>
            <div style={s.card}>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(130px,1fr))", gap: 12, marginBottom: 20 }}>
                {[["Citas hoy",todayAppts.length],["Pendientes",appointments.filter(a=>a.status==="pendiente").length],["Completadas",completedCount],["Canceladas",cancelledCount],["Rating",`⭐${doctor.rating}`]].map(([l,v],i)=>(
                  <div key={i} style={{ background:"rgba(82,183,136,0.06)", borderRadius:10, padding:"10px 14px" }}>
                    <div style={{ fontSize:11, color:"#74C69D" }}>{l}</div>
                    <div style={{ fontSize:20, fontWeight:700, color:"#52B788" }}>{v}</div>
                  </div>
                ))}
              </div>
              <button style={{ padding:"12px 28px", background:"linear-gradient(135deg,#2D6A4F,#52B788)", color:"#fff", border:"none", borderRadius:12, cursor:"pointer", fontSize:15, fontWeight:700, fontFamily:"inherit", opacity:aiLoading?0.7:1 }} onClick={getAiTip} disabled={aiLoading}>
                {aiLoading ? "Analizando... ⏳" : "🤖 Obtener consejo personalizado"}
              </button>
              {aiTip && <div style={{ background:"rgba(82,183,136,0.07)", border:"1px solid rgba(82,183,136,0.25)", borderRadius:12, padding:16, marginTop:16, fontSize:14, color:"#95D5B2", lineHeight:1.7, whiteSpace:"pre-wrap" }}>
                <div style={{ fontSize:12, color:"#52B788", fontWeight:700, marginBottom:8 }}>💡 CONSEJO IA</div>{aiTip}
              </div>}
            </div>
          </>
        )}
...
export default function App() {
  ...
  const [messages, setMessages] = useState([{ role: "assistant", content: "¡Hola! Soy tu asistente médico IA de Ayacucho. 🩺\n\nPuedo ayudarte a:\n• Encontrar el médico ideal para ti\n• Agendar una cita\n• Resolver dudas sobre síntomas\n\n¿En qué te puedo ayudar hoy?" }]);
  ...
  async function sendMessage() {
    ...
    try {
      ...
      const response = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 1000, system: `Eres un asistente médico IA amable para Ayacucho, Perú. Médicos: ${doctorList}. Responde en español, cálido. Si el usuario describe síntomas, sugiere especialidad y médico. Nunca diagnostiques. Máximo 3 párrafos.`, messages: [...messages, { role: "user", content: userMsg }].map(m => ({ role: m.role, content: m.content })) }) });
  ...
  async function submitRegistration() {
    ...
    try {
      ...
      const colors = ["#2D6A4F","#1B4332","#40916C","#52B788","#74C69D","#1A56DB"];
  ...
  const T = {
    app: { fontFamily: "'Crimson Pro', Georgia, serif", minHeight: "100vh", background: "linear-gradient(135deg, #0a1628 0%, #0d2137 50%, #0a1628 100%)", color: "#e8f4e8", position: "relative" },
    header: { position: "sticky", top: 0, zIndex: 100, background: "rgba(10,22,40,0.92)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(82,183,136,0.2)", padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64 },
    logo: { fontSize: 22, fontWeight: 700, color: "#52B788" },
    logoSub: { fontSize: 11, color: "#74C69D", letterSpacing: 3, textTransform: "uppercase", display: "block", marginTop: -4 },
    navBtn: (a) => ({ padding: "8px 16px", borderRadius: 8, border: a?"1px solid #52B788":"1px solid transparent", background: a?"rgba(82,183,136,0.15)":"transparent", color: a?"#52B788":"#95D5B2", cursor: "pointer", fontSize: 14, fontFamily: "inherit" }),
    section: { maxWidth: 1100, margin: "0 auto", padding: "40px 24px", position: "relative", zIndex: 1 },
    ctaPrimary: { padding: "14px 32px", background: "linear-gradient(135deg, #2D6A4F, #52B788)", color: "#fff", border: "none", borderRadius: 12, fontSize: 16, cursor: "pointer", fontWeight: 600, fontFamily: "inherit", boxShadow: "0 8px 32px rgba(82,183,136,0.3)" },
    ctaSecondary: { padding: "14px 32px", background: "transparent", color: "#52B788", border: "1.5px solid #52B788", borderRadius: 12, fontSize: 16, cursor: "pointer", fontFamily: "inherit" },
    grid: { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 20 },
    card: { background: "rgba(255,255,255,0.04)", border: "1px solid rgba(82,183,136,0.15)", borderRadius: 16, padding: 24, transition: "all 0.3s" },
    avatar: (c) => ({ width: 56, height: 56, borderRadius: 14, background: c||"#2D6A4F", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 700, color: "#fff", marginBottom: 16 }),
    filterBtn: (a) => ({ padding: "8px 18px", borderRadius: 20, border: "1px solid", borderColor: a?"#52B788":"rgba(82,183,136,0.3)", background: a?"rgba(82,183,136,0.2)":"transparent", color: a?"#52B788":"#74C69D", cursor: "pointer", fontSize: 13, fontFamily: "inherit" }),
    input: { width: "100%", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(82,183,136,0.25)", borderRadius: 10, padding: "10px 14px", color: "#e8f4e8", fontSize: 15, fontFamily: "inherit", outline: "none", boxSizing: "border-box" },
    timePill: (sel) => ({ padding: "6px 14px", borderRadius: 20, border: `1px solid ${sel?"#52B788":"rgba(82,183,136,0.3)"}`, background: sel?"rgba(82,183,136,0.2)":"transparent", color: sel?"#52B788":"#74C69D", cursor: "pointer", fontSize: 13, fontFamily: "inherit" }),
    label: { display: "block", fontSize: 13, color: "#74C69D", marginBottom: 6 },
    msgBubble: (r) => ({ maxWidth: "80%", padding: "12px 16px", borderRadius: r==="user"?"18px 18px 4px 18px":"18px 18px 18px 4px", background: r==="user"?"linear-gradient(135deg, #2D6A4F, #40916C)":"rgba(255,255,255,0.07)", alignSelf: r==="user"?"flex-end":"flex-start", fontSize: 15, lineHeight: 1.6, color: "#e8f4e8", whiteSpace: "pre-wrap", border: r==="user"?"none":"1px solid rgba(82,183,136,0.15)" }),
    planFeature: { padding: "8px 0", borderBottom: "1px solid rgba(82,183,136,0.1)", color: "#95D5B2", fontSize: 15, listStyle: "none" },
  };
    ...
    <div style={T.app}>
      ...
      <header style={T.header}>
        <div style={{ cursor: "pointer" }} onClick={() => setView("home")}>
          <span style={T.logo}>MediAyacucho</span>
        ...
        <>
          <div style={{ padding: "80px 24px 60px", textAlign: "center", position: "relative", zIndex: 1 }}>
            <h1 style={{ fontSize: "clamp(36px,6vw,72px)", fontWeight: 700, lineHeight: 1.1, margin: "0 0 20px" }}>
              Tu salud,<span style={{ color: "#52B788", display: "block" }}>nuestra prioridad</span>
            </h1>
            <p style={{ fontSize: 18, color: "#95D5B2", maxWidth: 560, margin: "0 auto 40px", lineHeight: 1.6 }}>Conectamos a la población de Ayacucho con médicos calificados. Agenda tu cita en minutos con IA.</p>
            ...
            <div style={{ display: "flex", gap: 40, justifyContent: "center", margin: "60px 0", flexWrap: "wrap" }}>
              {[[doctors.length||"6", "Médicos Certificados"],["500+","Citas Agendadas"],["4.8★","Calificación Promedio"],["24/7","Asistente IA"]].map(([n,l])=>(
                <div key={l} style={{ textAlign: "center" }}>
                  <span style={{ fontSize: 40, fontWeight: 700, color: "#52B788", display: "block" }}>{n}</span>
                  <span style={{ fontSize: 14, color: "#74C69D" }}>{l}</span>
                </div>
              ))}
            </div>
          </div>
          <div style={{ ...T.section, textAlign: "center" }}>
            <h2 style={{ fontSize: 28, fontWeight: 700, margin: "0 0 32px" }}>¿Cómo funciona?</h2>
            <div style={{ display: "flex", gap: 24, justifyContent: "center", flexWrap: "wrap" }}>
              {[["🤖","Consulta la IA","Cuéntanos tus síntomas y te recomendamos al médico ideal"],["📅","Elige tu cita","Selecciona el médico, fecha y hora disponible"],["📲","Recibe confirmación","WhatsApp instantáneo con dirección y todos los detalles"]].map(([icon,title,desc])=>(
                <div key={title} style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(82,183,136,0.15)", borderRadius:16, padding:28, maxWidth:240 }}>
                  <div style={{ fontSize: 40, marginBottom: 12 }}>{icon}</div>
                  <h3 style={{ margin:"0 0 8px", color:"#52B788", fontSize:18 }}>{title}</h3>
                  <p style={{ margin:0, color:"#74C69D", fontSize:14, lineHeight:1.6 }}>{desc}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {view === "doctors" && (
        <div style={T.section}>
          <h2 style={{ fontSize: 32, fontWeight: 700, margin: "0 0 8px" }}>Médicos en Ayacucho</h2>
          <p style={{ color: "#74C69D", margin: "0 0 24px" }}>
          ...
          {loadingDoctors ? (
            <div style={{ textAlign:"center", padding:60, color:"#52B788", fontSize:18 }}>⏳ Conectando a Supabase...</div>
          ) : (
            <div style={T.grid}>
              {filtered.map(doc => (
                <div key={doc.id} style={T.card}
                  onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-4px)";e.currentTarget.style.borderColor="rgba(82,183,136,0.4)";}}
                  onMouseLeave={e=>{e.currentTarget.style.transform="";e.currentTarget.style.borderColor="rgba(82,183,136,0.15)";}}>
                  <div style={T.avatar(doc.color)}>{doc.img || initials(doc.name)}</div>
                  <h3 style={{ fontSize:18, fontWeight:700, margin:"0 0 4px", color:"#e8f4e8" }}>{doc.name}</h3>
                  <p style={{ fontSize:13, color:"#74C69D", margin:"0 0 8px" }}>{doc.specialty}</p>
                  <div style={{ color:"#F4A261", fontSize:13 }}>{"★".repeat(Math.floor(doc.rating||5))} {doc.rating}</div>
                  <div style={{ marginTop:6, color:"#74C69D", fontSize:12 }}>{(doc.schedule||[]).join(" · ")}</div>
                  {doc.address && (
                    <div style={{ marginTop:8, display:"flex", alignItems:"flex-start", gap:6 }}>
                      <span style={{ fontSize:12 }}>📍</span>
                      <a href={doc.maps_url||"#"} target="_blank" rel="noreferrer" style={{ fontSize:12, color:"#52B788", textDecoration:"none", lineHeight:1.4 }}>{doc.address}</a>
                    </div>
                  )}
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:16 }}>
                    <div>
                      <span style={{ fontSize:20, fontWeight:700, color:"#52B788" }}>{doc.price}</span>
                      <span style={{ fontSize:12, color:"#74C69D" }}>/consulta</span>
                    </div>
                    {doc.available
                      ? <button style={{ padding:"8px 16px", background:"linear-gradient(135deg,#2D6A4F,#52B788)", color:"#fff", border:"none", borderRadius:10, cursor:"pointer", fontSize:13, fontWeight:600, fontFamily:"inherit" }} onClick={() => { setSelectedDoctor(doc); setView("booking"); }}>Agendar</button>
                  ...
                  <button style={{ marginTop:8, width:"100%", padding:"6px 0", background:"transparent", border:"1px solid rgba(82,183,136,0.2)", color:"#52B788", borderRadius:8, cursor:"pointer", fontSize:12, fontFamily:"inherit" }} onClick={() => setDashboardDoctor(doc)}>
                    📊 Ver mi dashboard
                  </button>
                </div>
              ))}
        ...
        <div style={T.section}>
          {confirmed ? (
            <div style={{ maxWidth:480, margin:"0 auto", textAlign:"center", padding:"40px 0" }}>
              <div style={{ fontSize:72, marginBottom:16 }}>🎉</div>
              <h2 style={{ color:"#52B788", fontSize:28, margin:"0 0 12px" }}>¡Cita Confirmada!</h2>
              <p style={{ color:"#74C69D", marginBottom:8 }}>Cita con <strong style={{ color:"#e8f4e8" }}>{selectedDoctor.name}</strong> el {bookingData.date} a las {bookingData.time}.</p>
              <p style={{ color:"#95D5B2", fontSize:14, marginBottom:32 }}>¿Deseas enviar la confirmación por WhatsApp?</p>
            ...
            <div style={{ maxWidth:540, margin:"0 auto" }}>
              <button onClick={() => setView("doctors")} style={{ background:"none", border:"none", color:"#52B788", cursor:"pointer", fontSize:15, marginBottom:24, padding:0, fontFamily:"inherit" }}>← Volver</button>
              <div style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(82,183,136,0.2)", borderRadius:16, padding:24, marginBottom:24 }}>
                <div style={{ display:"flex", gap:16, alignItems:"center" }}>
                  <div style={T.avatar(selectedDoctor.color)}>{selectedDoctor.img||initials(selectedDoctor.name)}</div>
                  <div>
                    <h3 style={{ margin:0, color:"#e8f4e8" }}>{selectedDoctor.name}</h3>
                    <p style={{ margin:"4px 0 0", color:"#74C69D", fontSize:14 }}>{selectedDoctor.specialty} · {selectedDoctor.price}</p>
                  </div>
                </div>
                {selectedDoctor.address && (
                  <div style={{ marginTop:16, padding:"12px 16px", background:"rgba(82,183,136,0.07)", borderRadius:10, display:"flex", alignItems:"flex-start", gap:10 }}>
                    <span style={{ fontSize:20 }}>📍</span>
                    <div>
                      <p style={{ margin:"0 0 4px", fontSize:12, color:"#52B788", textTransform:"uppercase", letterSpacing:0.5, fontWeight:700 }}>Dirección del consultorio</p>
                      <p style={{ margin:"0 0 6px", color:"#e8f4e8", fontSize:14 }}>{selectedDoctor.address}</p>
                      <a href={selectedDoctor.maps_url||"#"} target="_blank" rel="noreferrer" style={{ fontSize:12, color:"#52B788", textDecoration:"none", background:"rgba(82,183,136,0.15)", border:"1px solid rgba(82,183,136,0.3)", padding:"4px 12px", borderRadius:20, display:"inline-block" }}>🗺️ Ver en Google Maps</a>
                    </div>
                  </div>
                )}
              </div>
      ...
      {view === "chat" && (
        <div style={{ maxWidth:720, margin:"0 auto", padding:"40px 24px", position:"relative", zIndex:1 }}>
          ...
          <p style={{ color:"#74C69D", margin:"0 0 24px" }}>Cuéntame tus síntomas y te ayudo a encontrar el médico ideal</p>
          <div style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(82,183,136,0.2)", borderRadius:20, overflow:"hidden", display:"flex", flexDirection:"column", height:"70vh" }}>
            <div style={{ flex:1, overflowY:"auto", padding:24, display:"flex", flexDirection:"column", gap:16 }}>
              {messages.map((m,i)=>(
                <div key={i} style={T.msgBubble(m.role)}>
                  {m.role==="assistant" && <span style={{ fontSize:12, color:"#52B788", display:"block", marginBottom:4, fontWeight:600 }}>🩺 Asistente IA</span>}
                  {m.content}
                </div>
              ))}
              {loading && <div style={{ ...T.msgBubble("assistant"), color:"#52B788" }}>Pensando... ⏳</div>}
              <div ref={chatEndRef} />
            </div>
            <div style={{ display:"flex", gap:12, padding:16, borderTop:"1px solid rgba(82,183,136,0.15)" }}>
              <input style={{ flex:1, background:"rgba(255,255,255,0.06)", border:"1px solid rgba(82,183,136,0.25)", borderRadius:12, padding:"12px 16px", color:"#e8f4e8", fontSize:15, fontFamily:"inherit", outline:"none" }} placeholder="Escribe tu consulta aquí..." value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&sendMessage()} />
              <button style={{ padding:"12px 20px", background:"linear-gradient(135deg,#2D6A4F,#52B788)", color:"#fff", border:"none", borderRadius:12, cursor:"pointer", fontSize:20, fontFamily:"inherit" }} onClick={sendMessage} disabled={loading}>➤</button>
            </div>
          </div>
      ...
      {view === "doctor-register" && (
        <div style={{ maxWidth:600, margin:"0 auto", padding:"40px 24px", zIndex:1, position:"relative" }}>
          ...
          <p style={{ color:"#74C69D", margin:"0 0 24px" }}>Llega a más pacientes en Ayacucho</p>

          {regDone ? (
            <div style={{ background:"rgba(82,183,136,0.08)", border:"1px solid rgba(82,183,136,0.3)", borderRadius:16, padding:32, textAlign:"center" }}>
              <div style={{ fontSize:56, marginBottom:16 }}>✅</div>
              <h3 style={{ color:"#52B788", fontSize:22, margin:"0 0 8px" }}>¡Registro enviado!</h3>
              <p style={{ color:"#74C69D", fontSize:14 }}>Tu perfil está en revisión. Te activaremos en 24h tras verificar el pago.</p>
            </div>
          ) : (
            <div style={{ background:"rgba(255,255,255,0.04)", border:"2px solid rgba(82,183,136,0.3)", borderRadius:20, padding:32, marginBottom:24 }}>
              <div style={{ fontSize:24, fontWeight:700, color:"#52B788" }}>Plan Profesional</div>
              <div style={{ fontSize:48, fontWeight:700, color:"#e8f4e8", margin:"12px 0" }}>S/. 99<span style={{ fontSize:18, color:"#74C69D" }}>/mes</span></div>
              <ul style={{ listStyle:"none", padding:0, margin:"0 0 24px" }}>
                {["✅ Perfil médico verificado","✅ Gestión de citas online","✅ Panel de control completo","✅ Notificaciones WhatsApp/SMS","✅ Recomendaciones por IA","✅ Estadísticas en tiempo real","✅ Soporte prioritario 24/7"].map((f,i)=>(
                  <li key={i} style={{ padding:"8px 0", borderBottom:"1px solid rgba(82,183,136,0.1)", color:"#95D5B2", fontSize:15 }}>{f}</li>
                ))}
              </ul>
              ...
              <div style={{ marginBottom:14 }}>
                ...
                <p style={{ margin:"6px 0 0", fontSize:12, color:"#74C69D" }}>📍 Visible para tus pacientes al reservar</p>
              </div>
          ...
          <div style={{ background:"rgba(82,183,136,0.07)", border:"1px solid rgba(82,183,136,0.2)", borderRadius:16, padding:20, textAlign:"center" }}>
            <p style={{ color:"#74C69D", fontSize:13, margin:"0 0 12px" }}>🎯 ¿Quieres ver el panel de control antes de inscribirte?</p>
            <div style={{ display:"flex", gap:10, flexWrap:"wrap", justifyContent:"center" }}>
              {doctors.slice(0,3).map(doc=>(
                <button key={doc.id} style={{ padding:"8px 16px", background:"rgba(82,183,136,0.15)", border:"1px solid rgba(82,183,136,0.3)", borderRadius:10, color:"#52B788", cursor:"pointer", fontFamily:"inherit", fontSize:13 }} onClick={() => setDashboardDoctor(doc)}>
                  {doc.img||initials(doc.name)} {doc.name.split(" ")[1]}
                </button>
              ))}
            </div>
          </div>
          <p style={{ textAlign:"center", color:"#52B788", fontSize:14, marginTop:20 }}>¿Preguntas? WhatsApp: <strong>913 330 712</strong></p>
        </div>
      )}
    </div>
  );
